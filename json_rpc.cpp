...
boost::json::value call_impl_(C1& c1, R (C2::*pmf)(A...), boost::json::array const& args, std::index_sequence<I...>) {
...
boost::json::value call_impl(C1& c1, R (C2::*pmf)(A...), boost::json::array const& args) {
  if (args.size() != sizeof...(A)) { throw std::invalid_argument("Invalid number of arguments"); }

  return call_impl_(c1, pmf, args, std::index_sequence_for<A...>());
}

template <class C>
boost::json::value call(C& c, std::string_view method, boost::json::value const& args) {
  ...
  if (!found) { throw std::invalid_argument("Invalid method name"); }
