...
void test_context_helper(Context& context) {
  ...
  while (loop) {
    switch (context.read_offset()) {
      ...
      case 5:
        ...
        context.write(0);
        ASSERT_TRUE(std::string("Hello") == context.write_buffer());
  ...
  ASSERT_TRUE(std::string("World") == context.write_buffer());
...
TEST(minijson_reader_detail, parse_double_invalid) {
  ...
  ASSERT_THROW(minijson::detail::parse_double("1.0E+999"),
...
void read_quoted_string_invalid_helper(const char (&buffer)[Length],
...
TEST(minijson_reader_detail, read_quoted_string_invalid) {
  ...
  read_quoted_string_invalid_helper("\"\\h\"",
      minijson::parse_error::INVALID_ESCAPE_SEQUENCE,
      2,
      "Invalid escape sequence");
  read_quoted_string_invalid_helper("\"\\u0rff\"",
  ...
  read_quoted_string_invalid_helper("\"\\uD800a\"",
      minijson::parse_error::EXPECTED_UTF16_LOW_SURROGATE,
      7,
      "Expected UTF-16 low surrogate");
}
...
template <size_t Length>
void read_unquoted_value_invalid_helper(const char (&buffer)[Length],
    minijson::parse_error::error_reason expected_reason,
    size_t expected_offset,
    const char* expected_what) {
  bool exception_thrown = false;
...
TEST(minijson_reader, value_example) {
  ...
  ASSERT_TRUE("42.42" == value.as_string());
...
TEST(minijson_reader_detail, parse_unquoted_value_true) {
  ...
  ASSERT_TRUE("true" == value.as_string());
...
TEST(minijson_reader_detail, parse_unquoted_value_false) {
  ...
  const minijson::value value = minijson::detail::parse_unquoted_value(buffer_context);
  ASSERT_EQ(minijson::Boolean, value.type());
  ASSERT_TRUE("false" == value.as_string());
  ASSERT_EQ(0, value.as_long());
  ASSERT_FALSE(value.as_bool());
  ASSERT_DOUBLE_EQ(0.0, value.as_double());
}
...
TEST(minijson_reader_detail, parse_unquoted_value_null) {
  ...
  const minijson::value value = minijson::detail::parse_unquoted_value(buffer_context);
  ASSERT_EQ(minijson::Null, value.type());
  ASSERT_TRUE("null" == value.as_string());
  ASSERT_EQ(0, value.as_long());
  ASSERT_FALSE(value.as_bool());
  ASSERT_DOUBLE_EQ(0.0, value.as_double());
}
...
TEST(minijson_reader_detail, parse_unquoted_value_integer) {
  ...
  const minijson::value value = minijson::detail::parse_unquoted_value(buffer_context);
  ASSERT_EQ(minijson::Number, value.type());
  ASSERT_TRUE("42" == value.as_string());
  ASSERT_EQ(42, value.as_long());
  ASSERT_TRUE(value.as_bool());
...
TEST(minijson_reader_detail, parse_unquoted_value_double) {
  ...
  ASSERT_TRUE("42.0e+76" == value.as_string());
  ASSERT_EQ(0, value.as_long());
  ASSERT_FALSE(value.as_bool());
...
TEST(minijson_reader_detail, read_value_quoted_string) {
  ...
  ASSERT_TRUE("Hello world" == value.as_string());
  ASSERT_EQ(0, value.as_long());
  ASSERT_FALSE(value.as_bool());
  ASSERT_DOUBLE_EQ(0.0, value.as_double());
...
void parse_object_empty_handler(const char*, minijson::value) { FAIL(); }

void parse_array_empty_handler(minijson::value) { FAIL(); }
...
struct parse_object_single_field_handler : check_on_destroy_handler {
  ...
  void operator()(const char* field_name, const minijson::value& field_value) {
    ...
    ASSERT_TRUE("value" == field_value.as_string());
  }
};
...
TEST(minijson_reader, parse_object_multiple_fields) {
  char buffer[] = "{\"string\":\"value\\\"\\\\\\/"
                  "\\b\\f\\n\\r\\t\",\"integer\":42,\"floating_point\":"
                  "4261826387162873618273687126387,"
...
TEST(minijson_reader, parse_object_nested) {
...

  {
    ...
    minijson::parse_object(const_buffer_context,
        parse_object_nested_handler<minijson::const_buffer_context>(const_buffer_context));
  }
  {
    std::istringstream ss(buffer);
    minijson::istream_context istream_context(ss);
...
struct parse_array_single_elem_handler : check_on_destroy_handler {
  ...
  void operator()(const minijson::value& elem_value) {
    ...
    ASSERT_TRUE("value" == elem_value.as_string());
  }
};
...
struct parse_array_single_elem2_handler : check_on_destroy_handler {
  ...
  void operator()(const minijson::value& elem_value) {
    ...
    ASSERT_TRUE("1" == elem_value.as_string());
  }
};
...
TEST(minijson_reader, parse_array_nested) {
  ...
  {
    minijson::const_buffer_context const_buffer_context(buffer, sizeof(buffer) - 1);
    minijson::parse_array(const_buffer_context,
        parse_array_nested_handler<minijson::const_buffer_context>(const_buffer_context));
  }
  {
    std::istringstream ss(buffer);
    minijson::istream_context istream_context(ss);
  ...
  for (size_t i = sizeof(buffer) - 2; i < sizeof(buffer); i--) {
    ...
    try {
      ...
      switch (i) {
        ...
        case 12:
          ASSERT_EQ(parse_error::EXPECTED_COMMA_OR_CLOSING_BRACKET, e.reason());
...
template <size_t Length>
void parse_object_invalid_helper(const char (&buffer)[Length],
    minijson::parse_error::error_reason expected_reason,
    const char* expected_what = NULL) {
  minijson::const_buffer_context const_buffer_context(buffer, sizeof(buffer) - 1);
...
template <size_t Length>
void parse_object_invalid_helper2(const char (&buffer)[Length],
    minijson::parse_error::error_reason expected_reason,
    const char* expected_what = NULL) {
  minijson::const_buffer_context const_buffer_context(buffer, sizeof(buffer) - 1);
...
template <size_t Length>
void parse_array_invalid_helper(const char (&buffer)[Length],
    minijson::parse_error::error_reason expected_reason,
    const char* expected_what = NULL) {
  minijson::const_buffer_context const_buffer_context(buffer, sizeof(buffer) - 1);
...
template <size_t Length>
void parse_array_invalid_helper2(const char (&buffer)[Length],
    minijson::parse_error::error_reason expected_reason,
    const char* expected_what = NULL) {
  minijson::const_buffer_context const_buffer_context(buffer, sizeof(buffer) - 1);
...
TEST(minijson_dispatch, present) {
  ...
  minijson::dispatch("test2") << "test1" >> [&] { handled[0] = true; } << "test2" >>
      [&] { handled[1] = true; }  // should "break" here
          << "test3" >>
      [&] { handled[2] = true; } << "test2" >> [&] { handled[3] = true; };
...
TEST(minijson_dispatch, absent) {
  ...
  minijson::dispatch("x") << "test1" >> [&] { handled[0] = true; } << "test2" >> [&] { handled[1] = true; } << "test3" >>
      [&] { handled[2] = true; };
...
TEST(minijson_dispatch, absent_with_any_handler) {
  ...
  minijson::dispatch("x") << "test1" >> [&] { handled[0] = true; } << "test2" >> [&] { handled[1] = true; } << "test3" >>
      [&] { handled[2] = true; } << any >> [&] { handled[3] = true; };
...
TEST(minijson_dispatch, std_string) {
  ...
  minijson::dispatch(x) << x >> [&] { handled = true; };
...
TEST(minijson_dispatch, parse_object) {
  char json_obj[] = "{ \"field1\": 42, \"array\" : [ 1, 2, 3 ], \"field2\": \"asd\", "
                    "\"nested\" : { \"field1\" : 42.0, \"field2\" : true, \"ignored_field\" "
                    ": 0, \"ignored_object\" : {\"a\":[0]} },"
  ...
  parse_object(ctx, [&](const char* k, value v) {
    dispatch(k) << "field1" >> [&] { obj.field1 = v.as_long(); } << "field2" >>
        [&] { obj.field2 = v.as_string(); } << "nested" >>
        [&] {
          parse_object(ctx, [&](const char* k, value v) {
            dispatch(k) << "field1" >> [&] { obj.nested.field1 = v.as_double(); } << "field2" >>
                [&] { obj.nested.field2 = v.as_bool(); } << any >> [&] { ignore(ctx); };
          });
        } << "array" >>
        [&] { parse_array(ctx, [&](value v) { obj.array.push_back(v.as_long()); }); } << any >> [&] { ignore(ctx); };
  });
