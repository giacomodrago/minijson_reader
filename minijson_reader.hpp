...
class context_base : noncopyable {
 public:
  enum context_nested_status { NESTED_STATUS_NONE, NESTED_STATUS_OBJECT, NESTED_STATUS_ARRAY };
 ...
 public:
  context_base() : m_nested_status(NESTED_STATUS_NONE), m_nesting_level(0) {}

  context_nested_status nested_status() const { return m_nested_status; }
  ...
  void reset_nested_status() { m_nested_status = NESTED_STATUS_NONE; }

  void end_nested() {
    if (m_nesting_level > 0) { m_nesting_level--; }
  }
...
class buffer_context_base : public context_base {
 ...
 public:
  char read() {
    if (m_read_offset >= m_length) { return 0; }
  ...
  void new_write_buffer() { m_current_write_buffer = m_write_buffer + m_write_offset; }

  void write(char c) {
    if (m_write_offset >= m_read_offset) { throw std::runtime_error("Invalid write call, please file a bug report"); }

    m_write_buffer[m_write_offset++] = c;
  }

  const char* write_buffer() const { return m_current_write_buffer; }
...
class parse_error : public std::exception {
 ...
 public:
  template <typename Context>
  explicit parse_error(const Context& context, error_reason reason) : m_offset(get_offset(context)), m_reason(reason) {}
...
struct utf8_char {
  ...
  bool operator==(const utf8_char& other) const { return std::equal(bytes, bytes + sizeof(bytes), other.bytes); }

  bool operator!=(const utf8_char& other) const { return !operator==(other); }
};  // struct utf8_char
  ...
  while ((state != CLOSED) && ((c = context.read()) != 0)) {
    ...
    switch (state) {
      case OPENING_QUOTE:
        ...
        if (c != '"') { throw parse_error(context, parse_error::EXPECTED_OPENING_QUOTE); }
        state = CHARACTER;
      ...
      case UTF16_SEQUENCE:
        ...
        if (utf16_seq_offset == sizeof(utf16_seq) - 1) {
          ...
          } catch (const encoding_error&) { throw parse_error(context, parse_error::INVALID_UTF16_CHARACTER); }
      ...
      case CLOSED:  // to silence compiler warnings
        ...
        throw std::runtime_error("This line should never be reached, please "
                                 "file a bug report");  // LCOV_EXCL_LINE
    }
  }
...
char read_unquoted_value(Context& context, char first_char = 0) {
  if (first_char != 0) { context.write(first_char); }

  char c;

  while (((c = context.read()) != 0) && (c != ',') && (c != '}') && (c != ']') && !isspace(c)) { context.write(c); }

  if (c == 0) { throw parse_error(context, parse_error::UNTERMINATED_VALUE); }
...
void parse_object(Context& context, Handler handler) {
  ...
  if (nesting_level > MJR_NESTING_LIMIT) { throw parse_error(context, parse_error::EXCEEDED_NESTING_LIMIT); }
  ...
  while (state != END) {
    ...
    switch (state) {
      case OPENING_BRACKET:
        if (c != '{') { throw parse_error(context, parse_error::EXPECTED_OPENING_BRACKET); }
      ...
      case COLON:
        if (c != ':') { throw parse_error(context, parse_error::EXPECTED_COLON); }
        state = FIELD_VALUE;
        break;
    ...
    if (c == 0) {
      throw std::runtime_error("This line should never be reached, please file "
                               "a bug report");  // LCOV_EXCL_LINE
    }
  }
...
class dispatch : detail::noncopyable {
 ...
 public:
  explicit dispatch(const char* field_name) : m_field_name(field_name), m_handled(false) {}

  explicit dispatch(const std::string& field_name) : m_field_name(field_name.c_str()), m_handled(false) {}
...
class dispatch_rule {
 ...
 public:
  explicit dispatch_rule(dispatch& parent_dispatch, const char* field_name)
      : m_dispatch(parent_dispatch), m_field_name(field_name) {}
 ...
 public:
  ...
  void operator()() const {
    switch (m_context.nested_status()) {
      case Context::NESTED_STATUS_NONE: break;
      case Context::NESTED_STATUS_OBJECT: parse_object(m_context, *this); break;
      case Context::NESTED_STATUS_ARRAY: parse_array(m_context, *this); break;
    }
  }
};  // class ignore
