...
void process_stream(std::istream& is) {
  ...
  while (std::getline(is, line)) {
    ...
    if (result) {
      ...
      if (!match_found && !count_only && (file_count > 1)) { std::cout << current_file << ":\n"; }
      ++match_found;
      if (!count_only) {
        if (print_line_numbers) { std::cout << linenum << ":"; }
        if (print_byte_offset) { std::cout << what.position() << ":"; }
...
void process_file(const std::string& name) {
  ...
  if (is.bad()) { std::cerr << "Unable to open file " << name << std::endl; }
  process_stream(is);
}
...
int main(int argc, char* argv[]) {
  try {
    ...
    opts.add_options()("help,h", "produce help message")
        //("after-context,A", po::value<int>(&after_context)->default_value(0), "Print arg  lines  of  trailing  context
        //after  matching  lines. Places  a  line  containing  --  between  contiguous  groups  of matches.")
        //("before-context,B", po::value<int>(&before_context)->default_value(0), "Print  arg  lines  of  leading  context
        //before  matching lines. Places  a  line  containing  --  between  contiguous  groups  of matches.")
    ...
    if (vm.count("extended-regexp")) { flags = boost::regex_constants::extended; }
    if (vm.count("basic-regexp")) { flags = boost::regex_constants::basic; }
    if (vm.count("perl-regexp")) { flags = boost::regex_constants::perl; }
    if (vm.count("ignore-case")) { flags |= boost::regex_constants::icase; }
    if (vm.count("byte-offset")) { print_byte_offset = true; }
    if (vm.count("count")) { count_only = true; }
    if (vm.count("files-without-match")) { print_non_matching_files = true; }
    if (vm.count("files-with-matches")) { files_only = true; }
    if (vm.count("line-number")) { print_line_numbers = true; }
